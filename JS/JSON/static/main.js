...
let newDataUrl = 0; //глобальная переменная для новой url

function clickHandler(event) { 
    let input = document.querySelector('input'); //получаем то, что ввел пользователь в текстовую форму
    newDataUrl = "http://cat-facts-api.std-900.ist.mospolytech.ru/facts?q=" + input.value; //здесь записывается сам запрос 
    //по ключу q и введенному пользователем значению
    //если успешно, вызываем функцию downloadData(event.target) и загружаем первую страницу
    if (event.target) {
        downloadData(event.target);
        window.scrollTo(0, 0);
    }
}
...
window.onload = function() {
    ...
    searchbtn.addEventListener('click', clickHandler);//фиксирует нажатие на кнопку и вызывает функцию clickHandler()
}
